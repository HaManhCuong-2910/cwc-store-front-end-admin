import type { EStatusOrder, EStatusPaymentOrder, IDataOrderResponse } from '@/components/order/data'
...
export const ChangeStatusPaymentOrder = async (data: {
  id: string
  status: EStatusPaymentOrder
}) => {
  try {
    const res = await instance.put<TDefaultResponseObject>(
      '/cart/change-status-payment-order',
      data
    )
    return [res, null]
  } catch (err: any) {
